...
import com.funs.FunInterFace;
import com.funs.FunInterFace_Save;
import com.funs.RoleController_Save;
...
public class Tools {
	public static Object invokeFun(Object obj,String fun,Object args){
		try {
			Class clazz = obj.getClass();
	        Method m1= clazz.getDeclaredMethod(fun);
			return m1.invoke(obj, args);
		} catch (Exception e) {}
       return null;
	}
	public static Object invokeFun(Object obj,String fun){
		return invokeFun(obj,fun,null);
	}
	public static Object mySave(Object springNewObj,Object manager,BeanMapper beanMapper,
			FunInterFace_Save saveSetValueFun,
			FunInterFace_Save saveSetOldValueFun,
			FunInterFace roleController_Save){
	    Object dest = null;
		Long id = (Long)invokeFun(springNewObj,"getId");
		if (id != null) {
		    dest =invokeFun(manager,"get",id);
		    if(saveSetOldValueFun!=null)
		    		saveSetOldValueFun.execute();
		    beanMapper.copy(springNewObj, dest);
		} else {
		    dest = springNewObj;
		    if(saveSetValueFun!=null){
			    saveSetValueFun.setValue(dest);
			    saveSetValueFun.execute();
		    }
		}
		if(roleController_Save!=null)
			roleController_Save.execute();
		return dest;
	}
	public static void infoInput(HibernateEntityDao manager,Model model,Long id){
        if (id != null){
    			model.addAttribute("model", manager.get(id));
        }
	}
	public Page listForPage(TenantHolder tenantHolder,Pageable pageable,
			Map<String, Object> parameterMap,HibernateEntityDao manager,String tableName,String orderby){
		pageable.addFilter(parameterMap);
		if(tenantHolder!=null){
			String tenantId = tenantHolder.getTenantId();
			pageable.addFilter("tenantId", Operator.eq, tenantId);
		}
		pageable.setOrderProperty(orderby);
		pageable.setOrderDirection(Direction.desc);
		Page page=manager.findPage(pageable, tableName);
		return page;
	}
	public Page listForPage(TenantHolder tenantHolder,Pageable pageable,
			Map<String, Object> parameterMap,HibernateEntityDao manager,String tableName){
		return listForPage(tenantHolder,pageable,parameterMap,manager,tableName,"createTime");
	}
	public Page listForPage(Pageable pageable,
			Map<String, Object> parameterMap,HibernateEntityDao manager,String tableName,String orderby){
		return listForPage(null,pageable,parameterMap,manager,tableName,orderby);
	}
//
