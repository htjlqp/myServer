...
public class HibernateGenericDao extends HibernateBasicDao {
    ...
    public Object find(Pageable pageable, String tableName) {
    		List retlist= findList(pageable,tableName,"");
    		if(retlist!=null && retlist.size()>0){
    			return retlist.get(0);
    		}
    		return null;
    }
    public List findList(Pageable pageable, String tableName) {
    		return findList(pageable,tableName,"");
    }
    ...
    public Page findPage(Pageable pageable, String tableName){
    		return findPage(pageable,tableName,"");
    }
    public Page findPage(Pageable pageable, String tableName,String strGroupBy) {
        Query query = createTypedQuery(pageable, tableName, strGroupBy);
        Long total = findCount(pageable, tableName);
        Long totalPages = (long) Math.ceil(total.doubleValue()/pageable.getPageSize().doubleValue());
